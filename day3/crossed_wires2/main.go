...
func main() {
	...
	for p, c := range hitmap {
		if c > 1 {
			...
			steps = append(steps, aStepsMap[p] + bStepsMap[p])
		}
	}
...
func walk(input []string) ([]point, map[point]int) {
	hitmap := make(map[point]int)
	stepMap := make(map[point]int)
	traversedPoints := []point{}
	x, y := 0, 0
	stepCount := 0
	for _, i := range input {
		dir := string(i[0])
		...
		case "L":
			for j := 0; j < steps; j++ {
				...
				if _, exists := hitmap[newPoint]; !exists {
					traversedPoints = append(traversedPoints, newPoint)
					stepMap[newPoint] = stepCount
					hitmap[newPoint]++
				}
			}
		case "R":
	...
	return traversedPoints, stepMap
}
